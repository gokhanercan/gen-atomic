...
class ExperimentFactory(object):
    ...
    def __init__(
        self, lang_unit_name: str, default_prompting: PromptingBase = None
    ) -> None:
        super().__init__()
    ...
    def create_experiment_with_all_models(
        self, prompting: PromptingBase | None = None
    ) -> Experiment:
        ...
        mcs = ModelConfigurations(
            [ModelConfiguration(m, eff_prompting) for m in models]
        )
    ...
    def create_experiment_with_baseline_models(
        self, prompting: PromptingBase | None = None
    ) -> Experiment:
        unit: LangUnit = LangUnitFactory().Create(self.lang_unit_name)
        models: List[ModelBase] = ModelFactory().CreateModelsByFilters(
            ModelFilters(isBaseline=True)
        )
        eff_prompting: PromptingBase = prompting or self.default_prompting
        mcs = ModelConfigurations(
            [ModelConfiguration(m, eff_prompting) for m in models]
    ...
    def create_single_model_experiment(
        self, model_key: str, prompting: PromptingBase | None = None
    ) -> Experiment:
        unit: LangUnit = LangUnitFactory().Create(self.lang_unit_name)
        model: ModelBase = ModelFactory().CreateModelByKey(model_key)
        mcs: ModelConfigurations = ModelConfigurations(
            [ModelConfiguration(model, prompting or self.default_prompting)]
        )
        exp: Experiment = Experiment(unit, mcs)
        return exp
    ...
    def create_provider_experiment(
        ...
        unit: LangUnit = LangUnitFactory().Create(self.lang_unit_name)
        model_factory = ModelFactory()
        models: List[ModelBase] = model_factory.CreateModelsByFilters(
            ModelFilters(providerAbbr=provider_abbr)
        )
        if include_baselines:
            models += model_factory.CreateBaselineModels()
        mcs: ModelConfigurations = ModelConfigurations(
    ...
    def create_model_configurations_with_all_default_promptings(
        ...
        for p_meta in p_metas:
            p: PromptingBase = p_factory.create_prompting_instance(
                p_meta.key, lang_unit.CreateInfo()
            )
            if isinstance(p, DirectPrompting) and create_decorator_variations:
                ...
                for pd_comb in pd_combinations:
                    ...
                    for pd in pd_comb:
                        pd: PromptDecoratorBase = (
                            p_factory.create_prompt_decorator_instance(pd.key)
                        )
    ...
    def create_model_experiment_with_all_default_promptings(
        ...
        return Experiment(
            lang_unit,
            ModelConfigurations(
                self.create_model_configurations_with_all_default_promptings(
                    model_key, create_decorator_variations
                )
            ),
        )
...
class ExperimentFactoryTests(TestCase):
    ...
    def test_create_single_model_experiment__defaults_check_defaults(self):
        ...
        self.assertEqual(
            DirectPrompting, type(exp.model_configs.model_configs[0].prompting)
        )
